...
fn parse_args(app: App) -> ClientConfig {
    ...
    let config = matches
        .value_of("config")
        .expect("config path must be provided");
    let country = matches
        .value_of("COUNTRY")
        .expect("country must be provided");
    let question_file = matches
        .value_of("question")
        .expect("path to audio file with question to deliver notifications must be provided");
